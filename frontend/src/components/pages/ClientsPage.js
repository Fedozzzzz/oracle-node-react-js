...
class ClientsPage extends Component {
    ...
    onSubmitCreate = async () => {
        const {firstName, lastName, patherName, passportSeria, passportNum} = this.state;
        const body = {
            firstName,
            lastName,
            patherName,
    ...
    onSubmitModify = async () => {
        const {firstName, lastName, patherName, passportSeria, passportNum, selectedClient} = this.state;
        const body = {
    ...
    onShowBooksCount = async (clientId) => {
        try {
            ...
            this.setState({
                infoMessage: `This client has ${res.clientBooksCount !== 0 ? res.clientBooksCount : 'no'} books`,
            ...
            <tr key={cl.ID}>
                ...
                <th>
                    <button type="button" className="btn btn-outline-danger btn-sm mx-2"
                            onClick={() => this.onDelete(cl.ID)}>Delete
                    </button>
                    <button type="button" className="btn btn-outline-warning btn-sm mx-2"
                            onClick={() => this.onModify(cl)}>Edit
                    </button>
                    <button type="button" className="btn btn-warning btn-sm mx-2"
                            onClick={() => this.onShowBooksCount(cl.ID)}>Show Books Count
                    </button>
                    {/*<button type="button" className="btn btn-success btn-sm mx-2"*/}
                    {/*        onClick={() => this.onShowClientFine(cl.ID)}>Show Fine*/}
                    {/*</button>*/}
                </th>
            </tr>
        ));
        ...
        return (
            <div>
                ...
                <main role="main" className=" ml-sm-auto px-5 p-4">
                    ...
                    <button type="button" className="btn btn-outline-success my-3" onClick={this.onCreate}>
                        +Add client
                    </button>
