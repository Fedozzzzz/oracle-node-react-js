...
export default function BookTable(props) {
    const {books, onDelete, onModify, bookTypes, withoutTypes = false, readOnly = false} = props;
    ...
    function renderTableRows() {
        return books.map(bk => (
            <tr key={bk.ID}>
                ...
                {!readOnly && (<th>
                    {onDelete && (<button type="button" className="btn btn-outline-danger btn-sm mx-2"
                                          onClick={() => onDelete(bk.ID)}>Delete
                    </button>)}
                    {onModify && (
                        <button type="button" className="btn btn-outline-warning btn-sm "
                                onClick={() => onModify(bk)}>Edit
                        </button>)}
                </th>)}
            </tr>
        ));
